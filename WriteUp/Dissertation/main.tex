...
\section{Open Source}
The system was designed and documented so it could be reproduced by other rowing clubs and used to help prevent crashes. Due to plagiarism rules the repositories and documentation, including README, have not been made available. They will be made public immediately after my graduation. \\
...
The application thread was responsible for notifying the user when they are too close to an obstacle and allowing them to add obstacles. The initial state machine is shown in Figure 2.4. %The time to live (TTL) for each obstacle. If the object is permanent, such as a bridge, the TTL is set to $-1$, indicating that it should never be removed. 
\begin{figure}[h]
\begin{center}
...
%(TODO) Introduce and typeset the data structure to transfer information between the two threads 
...
\end{figure}
% ALso add the rest of the packet types and things like that 
% HELLO = const(0x00)
...
% what is the name of the thing we are trying to avoid when we say anti entropy is initiated by the lower ID node? 
...
% talk about what happens if these messages aren't sent in the correct time
% What if no messages? also adding how each node understands that all the DF messages have been sent  
%The plan for the system then requests that the message lists are 
% This change was made to minimise the number of messages sent, hopefully resulting in less bandwidth taken up and fewer error in transmissions 

% What if you are the receiving node?

% perhaps I should add a section for key data structures?
% timeouts and changes 
...
% What edge cases did we consider and deal with? 
...
% TODO talk about the easy thing on the data sett
...
%After some research, I discovered the haversine formula is commonly used for this application. TODO formula
...
A 10k resistor was chosen to be used in series with the buzzer. This resistance modulates the tone of the buzzer. A brief survey of rowers indicated the eventual tone to be most suitable as a warning. 
...
Such a system could be utilised for other applications. The most obvious is collision avoidance for other water sports, such as canoeing and kayaking. There are more avant-garde applications for this system, for instance decide if boats have collided within competitions, removing human bias. % Worry of low latency The MANET within the system could easily be ported to use in autonomous vehicles. There are more avant-garde applications for this system, for instance within Bumps to decide if boats have collided, removing human bias. 
